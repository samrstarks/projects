...
public class Table implements Iterable<Record>, Closeable {
  ...
  public class RIDPageIterator implements BacktrackingIterator<RecordId> {
    ...
    private ArrayBacktrackingIterator<RecordId> iter_array;

    public RIDPageIterator(Page page) {
      byte[] bitmap = getBitMap(page);
      ArrayList<RecordId> iters = new ArrayList<>();

      short entryNum = 0;
      for (; entryNum < numRecordsPerPage; ++entryNum) {
        if (Bits.getBit(bitmap, entryNum) == Bits.Bit.ONE) {
          iters.add(new RecordId(page.getPageNum(), entryNum));
        }
      }
      //assert(entryNum < numRecordsPerPage);
      iter_array = new ArrayBacktrackingIterator<>(iters.toArray(new RecordId[iters.size()]));
    }

    public boolean hasNext() {
      return iter_array.hasNext();
    }

    public RecordId next() {
      return iter_array.next();
    }
  ...
  public class RIDBlockIterator implements BacktrackingIterator<RecordId> {
    ...
    public RIDBlockIterator(BacktrackingIterator<Page> block) {
      this.block = block;
      Page curr_page = block.next();
      blockIter = new RIDPageIterator(curr_page);
    }
