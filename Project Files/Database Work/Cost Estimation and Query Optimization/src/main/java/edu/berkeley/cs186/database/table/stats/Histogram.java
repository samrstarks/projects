...
public class Histogram {
  ...
  public void buildHistogram(Table table, int attribute) {
    ...
    Iterator<Record> tIter= table.iterator();

    float curr;
    while (tIter.hasNext()) {
      curr = quantization(tIter.next(), attribute);
      if (curr < this.minValue) {
        this.minValue = curr;
      }
    ...
    //2. calculate the width of each bin -- (max - min)/#buckets
    this.width = (this.maxValue - this.minValue) / (float)this.numBuckets;

    //3. create each bucket object
    float left = minValue;
    for (int i = 0; i < numBuckets; i++) {
      float right = left + width;
      this.buckets[i] = new Bucket<>(left, right);
      left += width;
    }

    //4. Populate the buckets by incrementing;
    for (Record record: table) {
      curr = quantization(record, attribute);
      for (int i = 0; i < this.numBuckets; i++) {
        if (curr >= this.buckets[i].getStart() && curr < this.buckets[i].getEnd()) {
          buckets[i].increment(curr);
          break;
        } else {
          if (i == this.numBuckets - 1 && curr >= buckets[i].getStart() && curr <= buckets[i].getEnd()) {
            buckets[i].increment(curr);
          }
        }
      }
  ...
  private float [] allEquality(float qvalue) {
    ...
    for (int i = 0; i < this.numBuckets; i++) {
      if (qvalue >= this.buckets[i].getStart() && qvalue < this.buckets[i].getEnd()) {
        ...
        if (i == numBuckets - 1 && qvalue >= this.buckets[i].getStart() && qvalue <= this.buckets[i].getEnd()) {
          result[i] = (float)1 / this.buckets[i].getDistinctCount();
        } else {
          result[i] = 0;
        }
 ...
 /**
  ...
  private float [] allNotEquality(float qvalue){
    ...
    for (int i = 0; i < this.numBuckets; i++) {
      if (qvalue >= this.buckets[i].getStart() && qvalue < this.buckets[i].getEnd()) {
        result[i] = (float)1 - ((float)1 / this.buckets[i].getDistinctCount());
      } else {
        if (i == numBuckets - 1 && qvalue >= this.buckets[i].getStart() && qvalue <= this.buckets[i].getEnd()) {
          result[i] = (float)1 - ((float)1 / this.buckets[i].getDistinctCount());
        } else {
  ...
  private float [] allGreaterThan(float qvalue){
    ...
    for (int i = 0; i < this.numBuckets; i++) {
      if (qvalue >= this.buckets[i].getStart() && qvalue < this.buckets[i].getEnd()) {
        ...
        if (i == this.numBuckets - 1 && qvalue >= this.buckets[i].getStart() && qvalue < this.buckets[i].getEnd()) {
          result[i] = (this.buckets[i].getEnd() - qvalue) / this.width;
        } else {
          if (qvalue >= this.buckets[i].getEnd()) {
            result[i] = 0;
          }
