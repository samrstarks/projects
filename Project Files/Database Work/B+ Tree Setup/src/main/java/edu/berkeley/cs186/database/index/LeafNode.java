...
class LeafNode extends BPlusNode {
  ...
  public Optional<Pair<DataBox, Integer>> put(DataBox key, RecordId rid)
    ...
    //iterate through to find right spot to insert
    ...
    //handle splitting nodes if needed:
    if (keys.size() > (2 * metadata.getOrder())) {
      //create new key and rids arraylist:
      List<DataBox> r_keys = new ArrayList<>(keys.subList(this.metadata.getOrder(), keys.size()));
      List<RecordId> r_rids = new ArrayList<>(rids.subList(this.metadata.getOrder(), keys.size()));

      //use r_keys and r_rids to make right leaf for splitting:
      LeafNode right_leaf = new LeafNode(metadata, r_keys, r_rids, rightSibling);

      //update og (left) leafnode:
      ...
      //set Right sibling of old node:
      rightSibling = Optional.of(right_leaf.getPage().getPageNum());

      //create pair to return (right_leaf, right_key):
      Pair<DataBox, Integer> pair = new Pair(r_keys.get(0), this.rightSibling.get());
      sync();
  ...
  public static LeafNode fromBytes(BPlusTreeMetadata metadata, int pageNum) {
    ...
    //make arraylists for keys and rids
    List<DataBox> keys = new ArrayList<>();
    List<RecordId> rids = new ArrayList<>();

    //get page id of sibling
    Optional<Integer> rightSibling = Optional.empty();
    int rpid = buf.getInt();

    if (rpid != -1) {
      rightSibling = Optional.of(rpid);
    }
