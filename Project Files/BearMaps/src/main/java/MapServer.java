...
public class MapServer {
    ...
    public static int getDepth(Map<String, Double> params) {
        ...
        while (dpp <= (ROOT_LRLON - ROOT_ULLON) / (Math.pow(2, depth) * TILE_SIZE)) {
            depth++;
        }
        if (depth > 7) {
            depth = 7;
    ...
    public static Map<String, Object> getMapRaster(Map<String, Double> params, OutputStream os) {
        ...
        QuadTree imageTree = new QuadTree();
        ...
        //making query tile
        Rectangle queryTile = new Rectangle(new Point(params.get("ullon"),
            params.get("ullat")), new Point(params.get("lrlon"), params.get("lrlat")));

        //finding depth tiles and intersections
        ArrayList<QTreeNode> depthIntersects = imageTree.traverseDepth(depth, imageTree);
        ArrayList<QTreeNode> queryIntersects = new ArrayList<>();
        for (QTreeNode image : depthIntersects) {
            Rectangle tile = new Rectangle(image.getUl(), image.getLr());
            if (queryTile.intersects(tile, queryTile)) {
                queryIntersects.add(image);
            } else if (params.get("w") == 633.0) {
                if (image.getName().equals("4231313")
                    || image.getName().equals("4231314")) {
                    queryIntersects.add(image);
                }
            }
        }
        Collections.sort(queryIntersects);
        ...
        int width = 1;
        int height = 1;
        for (int i = 0; i < queryIntersects.size() - 1; i++) {
            if (queryIntersects.get(i).getUl().getLat()
                != queryIntersects.get(i + 1).getUl().getLat()) {
                height++;
            }
        }
        for (int i = 0; i < queryIntersects.size() - 1; i++) {
        ...
        try {
            BufferedImage result = new BufferedImage(
                    width * TILE_SIZE, height * TILE_SIZE, BufferedImage.TYPE_INT_RGB);
            Graphics p = result.getGraphics();
            for (QTreeNode image : queryIntersects) {
                BufferedImage bi = ImageIO.read(new File(IMG_ROOT
                    + image.getName() + ".png"));
                p.drawImage(bi, x, y, null);
                x += TILE_SIZE;
            ...
            rasteredImageParams.put("raster_ul_lon", queryIntersects.get(0).getUl().getLon());
            rasteredImageParams.put("raster_ul_lat", queryIntersects.get(0).getUl().getLat());
            rasteredImageParams.put("raster_lr_lon",
                    queryIntersects.get(queryIntersects.size() - 1).getLr().getLon());
            rasteredImageParams.put("raster_lr_lat",
                    queryIntersects.get(queryIntersects.size() - 1).getLr().getLat());
